...
GST has a worst case complexity of \(O(n^3)\)\autocite[p. 5]{GST} and a best case complexity of \(O(n^2)\)\autocite[p. 14]{JPlagP}.

- Uses Karp-Robin -> average O(n pow 2)
- all hash values for substring with a length of MML are computed for both strings
- each hash value from A is compared with each value from B, if two values are the same possible match beginning at
that token has been found
- possible match verified token by token
- attempt to expand match as far as possible
- Quadratic number of hashes need to be compared -> a hash table is used for locating the substring from B -> liner amount of steps

- Further optimizations
- token by token comparison backwards after maxmatch -> all matches below maxmatches not relevant
- A is always chosen to be shorter than B -> A always traversed completely B only accessed through hash table

- worst case still O(n pow 3) average case O(n) \autocite[p. 14]{JPlagP}
\subsubsection{Preliminary results}
